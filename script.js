...
document.addEventListener("DOMContentLoaded", function() {
  ...
  for (var i = 0; i < tiles.length; i++) {
    tiles[i].addEventListener('click', function(e){handleTileClick(e)}, false);
  }
...
function handleTileClick( e ){
  ...
  scoreContainer.innerHTML = "Score: <span id='score'>"+currentScore+"</span>/<span>"+totalQuestions+"</span>"
...
function addAnswerButtons(answers){
  ...
  for(i = 0; i<answerButtons.length; i++)
  {
    answerButtons[i].addEventListener("click", async function(e){
      assessment = validateAnswer(questions[currentQuestion].answers[this.id].correct);
...
function endScreen(){
  ...
  // modularity
  string = "<div id='endOverlay'> <div class='endBox'><p>Quiz complete!</p> <p>You scored "+currentScore+"/"+totalQuestions+"!</p> <div><button id='newTopic'>New Topic</button><button id='restartQuiz'>Try Again</button></div> </div> ";
  document.body.innerHTML += string;
  let newTopic = document.getElementById("newTopic");
  newTopic.addEventListener("click", ()=>{window.location.reload();}, false);
  let restart = document.getElementById("restartQuiz");
  restart.addEventListener("click", ()=>{currentScore = 0; currentQuestion = 0; removeOverlay(); nextQuestion();}, false);
}
...
function categoryQuestions( category, json ){
  ...
  let selected = randomised.slice(0, 5);
